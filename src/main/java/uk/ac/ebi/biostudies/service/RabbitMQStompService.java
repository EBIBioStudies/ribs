...
public class RabbitMQStompService {
    ...
    private class RabbitMQStompSessionHandler extends StompSessionHandlerAdapter {
        ...
        public void afterConnected(StompSession session, StompHeaders connectedHeaders) {
            String submissionPartialQueue = env.getProperty("partial.submission.rabbitmq.queue", String.class, "submission-submitted-partials-queue");
            try {
                submissionPartialQueue += new StringBuilder().append("-").append(new BufferedReader(new InputStreamReader(
                       Runtime.getRuntime().exec("hostname").getInputStream())).readLine().trim()).toString();
            } catch (IOException e) {
                e.printStackTrace();
            }
