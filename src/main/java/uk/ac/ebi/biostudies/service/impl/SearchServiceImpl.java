...
public class SearchServiceImpl implements SearchService {
  ...
  @Autowired EFOExpandedHighlighter efoExpandedHighlighter;
  @Autowired FacetService facetService;
  @Autowired AnalyzerManager analyzerManager;
  @Autowired SecurityQueryBuilder securityQueryBuilder;
  ...
  void init() {
    ...
    statsCache = CacheBuilder.newBuilder().expireAfterAccess(720, TimeUnit.MINUTES).build();
  ...
  private ObjectNode applySearchOnQuery(
      ...
      sort =
          new Sort(
              ...
              new SortedNumericSortField(
                  Fields.MODIFICATION_TIME, SortField.Type.LONG, shouldReverse));
    ...
    try {
      ...
      pageSize =
          Math.min(
              pageSize,
              currentUser != null && currentUser.isSuperUser() ? Integer.MAX_VALUE : MAX_PAGE_SIZE);
      ...
      boolean isTotalHitsExact =
          hits.totalHits == null || hits.totalHits.relation.equals(TotalHits.Relation.EQUAL_TO);
        ...
        for (int i = (page - 1) * pageSize; i < to; i++) {
          ...
          for (String field : indexManager.getIndexEntryMap().keySet()) {
            ...
            if (fieldData.has(IndexEntryAttributes.RETRIEVED)
              ...
              if (fieldData
                      .get(IndexEntryAttributes.FIELD_TYPE)
                      .asText()
                      .equalsIgnoreCase(IndexEntryAttributes.FieldTypeValues.LONG)
                  && doc.get(field) != null) {
          ...
          if (doHighlight) {
            docNode.put(
                Fields.CONTENT,
                efoExpandedHighlighter.highlightQuery(
                    query, Fields.CONTENT, doc.get(Fields.CONTENT), true));
          }
          docs.add(docNode);
        }
  ...
  private SortField.Type extractFieldType(String sortBy) {
    try {
      if (sortBy == null || sortBy.isEmpty() || sortBy.equalsIgnoreCase("relevance"))
        ...
        return field.get("fieldType").asText().toLowerCase().contains("str")
  ...
  private boolean extractSortOrder(String sortOrder, String sortBy) {
    if (sortOrder.isEmpty()) {
      sortOrder =
          (Fields.ACCESSION.equalsIgnoreCase(sortBy)
                  || Fields.TITLE.equalsIgnoreCase(sortBy)
                  || Fields.AUTHOR.equalsIgnoreCase(sortBy))
              ? SortOrder.ASCENDING
  ...
  public String search(
      String queryString,
      JsonNode selectedFacetsAndFields,
      String prjName,
      int page,
      int pageSize,
      String sortBy,
    ...
    if (queryString.isEmpty() && sortBy.isEmpty()) {
      sortBy =
          Fields.RELEASE_DATE; // TODO: Make sure RELEASE_TIME is used correctly all over the code
      // base
      doHighlight = false;
    ...
    ObjectMapper mapper = new ObjectMapper();
    ObjectNode response = mapper.createObjectNode();
    JsonNode selectedFields =
        selectedFacetsAndFields == null ? null : selectedFacetsAndFields.get("fields");
    JsonNode selectedFacets =
        selectedFacetsAndFields == null ? null : selectedFacetsAndFields.get("facets");
    Pair<Query, EFOExpansionTerms> resultPair =
        queryService.makeQuery(queryString, prjName, selectedFields);
    try {
      ...
      response =
          applySearchOnQuery(
              queryAfterSecurity, page, pageSize, sortBy, sortOrder, doHighlight, queryString);

      // add expansion
      EFOExpansionTerms expansionTerms = resultPair.getValue();
      if (expansionTerms != null
          && expansionTerms.efo.size() + expansionTerms.synonyms.size()
              > EFOQueryExpander.MAX_EXPANSION_TERMS) {
  ...
  @Override
  public InputStreamResource getStudyAsStream(
      String accession,
      String relativePath,
      boolean anonymise,
      ...
      String secretKey)
      throws IOException {
    return getStudyAsStream(
        accession, relativePath, anonymise, storageMode, true, isPublicStudy, secretKey);
  }
  ...
  public InputStreamResource getStudyAsStream(
    ...
    try {
      if (inputStream == null) {
        switch (storageMode) {
          case FIRE:
            inputStream =
                fireService.cloneFireS3ObjectStream(relativePath + "/" + accession + ".json");
            break;
          default:
            if (!isPublicStudy) {
              relativePath =
                  StudyUtils.modifyRelativePathForPrivateStudies(secretKey, relativePath);
            }
            inputStream =
                new FileInputStream(
                    Paths.get(indexConfig.getFileRootDir(), relativePath, accession + ".json")
                        .toFile());
        }
        if (inputStream
            != null) { // cache miss, we do not have this pagetab in index so we will add it
    ...
    if (inputStream == null) throw new IOException(codeIOException);
  ...
  private void removePagetabSensitiveSections(JsonNode json) {
    ...
    if (subSections != null && subSections.isArray()) {
      ...
      while (iterator.hasNext()) {
        ...
        if (node.has("type")
            && sectionsToFilter.contains(node.get("type").textValue().toLowerCase())) {
          iterator.remove();
        }
      }
    }
  ...
  private InputStream getPagetabFromPagetabIndex(String accession) throws Exception {
    TopDocs pagetabResult =
        indexManager
            .getPagetabIndexSearcher()
            .search(new TermQuery(new Term(Constants.Fields.ACCESSION, accession)), 1);
  ...
  public String getFieldStats() throws Exception {
    ...
    if (responseString == null) {
      ...
      DocValuesStats.SortedLongDocValuesStats fileStats =
          new DocValuesStats.SortedLongDocValuesStats("files");
      searcher.search(new MatchAllDocsQuery(), new DocValuesStatsCollector(fileStats));
      response.put("files", fileStats.sum());

      DocValuesStats.SortedLongDocValuesStats linkStats =
          new DocValuesStats.SortedLongDocValuesStats("links");
  ...
  public ObjectNode getSimilarStudies(String accession, String secretKey) throws Throwable {
    ...
    if (secretKey != null) return result;
    int maxHits = 4;
    ...
    for (int i = 0; i < mltDocs.scoreDocs.length; i++) {
      ...
      study.set(
          ...
          mapper.valueToTree(
              indexManager
                  .getSearchIndexReader()
                  .document(mltDocs.scoreDocs[i].doc)
                  .get(Fields.ACCESSION)));
      study.set(
          Fields.TITLE,
          mapper.valueToTree(
              indexManager
  ...
  @Override
  public Document getDocumentByAccessionAndType(String accession, String secretKey, String type)
      throws SubmissionNotAccessibleException {
    Document document = getDocumentByAccession(accession, secretKey);
    if (document == null || !document.get(Fields.TYPE).equalsIgnoreCase(type.trim())) return null;
    return document;
  }
  ...
  public String getLatestStudies() throws Exception {
    ...
    if (!isLoggedIn) {
      ...
      if (responseString == null) {
        responseString =
            search(
                URLDecoder.decode(Constants.Fields.TYPE + ":study", String.valueOf(UTF_8)),
                null,
                null,
                1,
                5,
                Constants.Fields.RELEASE_TIME,
  ...
  public boolean isDocumentInCollection(Document submissionDoc, String collection) {
    return StringUtils.isEmpty(collection)
        || submissionDoc.get("collection").contains(collection.toLowerCase());
  }
}
