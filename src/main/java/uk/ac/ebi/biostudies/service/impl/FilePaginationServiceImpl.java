...
public class FilePaginationServiceImpl implements FilePaginationService {
  ...
  public ObjectNode getStudyInfo(String accession, String secretKey)
    ...
    for (String att : allAtts) {
      if (att.isEmpty() || headerSet.contains(att)) continue;
      headerSet.add(att);
      orderedList.add(att);
    }
    int counter = 0;
    ...
    if (doc.get(Constants.Fields.HAS_FILE_PARSING_ERROR) != null) {
    ...
    try {
      if (sectionsWithFiles != null) {
        studyInfo.set(
            "sections",
            mapper.readTree("[\"" + sectionsWithFiles.replaceAll(" ", "\",\"") + "\"]"));
      }
    } catch (Exception e) {
  ...
  private void setDates(ObjectNode studyInfo, Document doc) {
    try {
      ...
      logger.error(
          "Invalid release date for {}: {} ",
          doc.get(Constants.Fields.ACCESSION),
          doc.get(Constants.Fields.RELEASE_TIME));
    }
    ...
    try {
      ...
      logger.error(
          "Invalid modification date for {}: {} ",
          doc.get(Constants.Fields.ACCESSION),
          doc.get(Constants.Fields.MODIFICATION_TIME));
    }
  }
  ...
  private Document getFileDocument(String accession, String path)
    ...
    queryBuilder.add(
        keywordParser.parse(Constants.File.OWNER + ":" + accession), BooleanClause.Occur.MUST);
    queryBuilder.add(
        keywordParser.parse(Constants.File.PATH + ":" + StudyUtils.escape(path)),
        BooleanClause.Occur.MUST);
    TopDocs hits = searcher.search(queryBuilder.build(), Integer.MAX_VALUE);
    return hits.totalHits.value > 0
        ? searcher.getIndexReader().document(hits.scoreDocs[0].doc)
        : null;
  }
   ...
   * private study data.
   *
   * <p>This method checks access permissions using {@link #canShareStudy(ObjectNode, Document,
   ...
   * @param doc the Lucene document containing the indexed study data, including the secret key
   *     field
