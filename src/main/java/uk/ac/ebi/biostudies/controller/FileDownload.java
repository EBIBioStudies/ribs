...
public class FileDownload {
  ...
  @Autowired ZipDownloadService zipDownloadService;
  ...
  @Autowired BatchDownloadScriptBuilder batchDownloadScriptBuilder;

  @RequestMapping(
      value = {"/files/**", "/{collection}/files/**"},
      method = RequestMethod.POST)
  public void getFilesInZippedFormat(
    ...
    if (storageMode == Constants.File.StorageMode.NFS && !StudyUtils.isPublicStudy(submissionDoc))
      relativeBaseDir =
          StudyUtils.modifyRelativePathForPrivateStudies(
              submissionDoc.get(Constants.Fields.SECRET_KEY), relativeBaseDir);
    ...
    if (dlType.equalsIgnoreCase("zip"))
      ...
      response.addHeader(
          "Content-Disposition",
          "attachment; filename=" + accession + "-" + os + "-" + dlType + "." + fileExtension);
      response.addHeader("Cache-Control", "no-cache");
      response
          .getWriter()
          .print(
              batchDownloadScriptBuilder.fillTemplate(
                  dlType, Arrays.asList(files), relativeBaseDir, os, storageMode));
  ...
  private Document getFilePaths(HttpServletRequest request, HttpServletResponse response)
      throws Exception {
    String[] args =
        request
            .getRequestURI()
            .replaceAll(request.getContextPath() + "(/[a-zA-Z])?/files/", "")
            .split("/");
  ...
  private String getFileExtension(String os) {
    if (os.equalsIgnoreCase("windows")) return "bat";
    else return "sh";
  }
  ...
  private String[] createFireCompatibleFileNames(String[] inputFileNames) {
    String[] response =
        Arrays.stream(inputFileNames)
            .map(name -> StudyUtils.decodeForFireBug(name))
            .toArray(String[]::new);
    return response;
  }
}
