...
public class View {
  ...
  @RequestMapping(
      value = {"/studies", "/studies/", "/{collection}/studies", "/{collection}/studies/"},
  ...
  public ModelAndView detail(
    ...
    var mav = new ModelAndView();
    String inferredType =
        accession.toUpperCase().startsWith("A-")
            ? "arrays"
            : accession.toUpperCase().startsWith("C-") ? "compounds" : "studies";
    String viewName = "detail";
    if (collection == null
        && (accession.toUpperCase().startsWith("E-") || accession.toUpperCase().startsWith("A-"))) {
      viewName =
          String.format(
              "redirect:/arrayexpress/%s/%s" + (key != null ? "?key=" + key : ""),
              inferredType,
              accession);
    } else if (collection == null
        && request.getRequestURL().toString().contains("/studies/")
        && !inferredType.equalsIgnoreCase("studies")) {
      viewName =
          String.format(
              "redirect:/%s/%s" + (key != null ? "?key=" + key : ""), inferredType, accession);
    } else {
      mav.addObject("collection", collection);
  ...
  @RequestMapping(
      ...
      method = RequestMethod.GET)
  public ModelAndView genericStudyView(
      @PathVariable(required = false) String collection,
  ...
  public ModelAndView genericView(@PathVariable String view) throws Exception {
    ...
    if (servletContext.getResource("jsp/" + view.toLowerCase() + ".jsp") == null) {
      Document collection =
          searchService.getDocumentByAccessionAndType(
              view, null, Constants.SubmissionTypes.COLLECTION);
      if (collection != null) {
  ...
  public RedirectView redirectEMPIAR(
      @PathVariable(required = false) String collection, @PathVariable String id) {
